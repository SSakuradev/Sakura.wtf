...
getgenv().Sakura_set_antidetect = function(v)
...
local CUSTOM_UI_FONTS = {
    ...
    ['Tahoma'] = {
        ...
        AltFontFiles = { 'Sakura/Tahoma.font', 'library/Tahoma.font', 'Tahoma.ttf', 'library/Tahoma.ttf' },
    },
};
...
function Library:SetGradientText(Label, Text)
    ...
    local gradient = Label:FindFirstChild('SakuraTextGradient');
    if not gradient then
        gradient = Library:Create('UIGradient', {
            Name = 'SakuraTextGradient';
            Parent = Label;
        });
...
function Library:UpdateMenuBlur()
    ...
    local blur = lighting:FindFirstChild('SakuraMenuBlur');
...
getgenv().Sakura_wm = getgenv().Sakura_wm or { ping = true, executor = false, fps = true, time = true };
task.spawn(function()
    local fps = 60;
    local execName = nil;
    ...
    while task.wait(0.5) do
        if Library.Watermark and Library.Watermark.Visible then
            local wm = getgenv().Sakura_wm or {};
            local parts = { "Sakura.lol" };
...
function Library:SaveSoundSettings()
    ...
    local SettingsPath = (self.SoundSettingsFolder or 'Sakura') .. '/settings/sounds.json';
    local Data = {
...
function Library:InitSoundSettings(Folder)
    ...
    self.SoundSettingsFolder = Folder or 'Sakura';
...
function Library:InitSoundAssets()
    local Folder = self.SoundSettingsFolder or 'Sakura';
    ...
    if not self.ToggleSound then
        ...
        if ToggleAsset then
            ...
            self.ToggleSound.Name = 'SakuraToggleSound';
    ...
    if not self.NotificationSound then
        ...
        if NotifyAsset then
            ...
            self.NotificationSound.Name = 'SakuraNotificationSound';
    ...
    if not self.SliderSound then
        ...
        if SliderAsset then
            ...
            self.SliderSound.Name = 'SakuraSliderSound';
...
function Library:CreateWindow(...)
    ...
    do
        local saved = getgenv().Sakura_ui_size;
        if not saved then
            pcall(function()
                if isfile and isfile('Sakura/ui_size.txt') then
                    local d = tostring(readfile('Sakura/ui_size.txt'));
    ...
    do
        ...
        local function persistSize()
            ...
            getgenv().Sakura_ui_size = { math.floor(s.X), math.floor(s.Y) };
            pcall(function()
                if writefile then
                    if makefolder and isfolder and not isfolder('Sakura') then makefolder('Sakura') end;
                    writefile('Sakura/ui_size.txt', math.floor(s.X) .. ',' .. math.floor(s.Y));
                end;
            end);
        end;
    ...
    local function UpdateTitle()
        local rawTitle = Config.Title or "Sakura.lol";
...
local ThemeManager = {} do
	...
	ThemeManager.DefaultTheme = 'Sakura';
	ThemeManager.BuiltInThemes = {
		...
		['Sakura'] 	    = { 65, httpService:JSONDecode('{"FontColor":"e0d4d6","MainColor":"121212","AccentColor":"a65d67","SelectedTabColor":"0a0a0a","BackgroundColor":"0a0a0a","OutlineColor":"000000","GradientColor":"6b3038"}') },
	};
	...
	function ThemeManager:LoadDefault()
		local theme = self.DefaultTheme or 'Sakura';
	...
	function ThemeManager:CreateAppearanceManager(groupbox, skipLoadDefault)
		...
		groupbox:AddDropdown('ThemeManager_ThemeList', { Text = 'Presets', Values = ThemesArray, Default = 'Sakura' });
...
local SaveManager = (function()
    ...
    SaveManager.Folder = 'Sakura/rivals';
    SaveManager.Ignore = {};
    ...
    function SaveManager:Save(name)
        ...
        do
            data.widgets = {};
            local ws = getgenv().Sakura_widgets;
...
local Window = Library:CreateWindow({
    Title = 'Sakura.lol - https://Sakura.lol/ [KEYLESS]',
    AutoShow = false,
    BackgroundImage = "",
    SubTitle = "Rivals",
    Center = true,
...
task.spawn(function()
    for name, suffix in font_files do
        local ttf_path = "Sakura/rivals/fonts/" .. suffix;
        local font_path = "Sakura/rivals/fonts/" .. name .. ".font";
...
do
    ...
    getgenv().Sakura_apply_opt = function(kind)
    ...
    local FFLAG_PRESET = [==[
{
  ...
  "FIntEnableCullableScene2HundredthPercent3": "100"
    ...
    getgenv().Sakura_fflag_preset = FFLAG_PRESET;
    getgenv().Sakura_inject_fflags = function(json_str)
        ...
        if writefile then
            pcall(function()
                if makefolder and isfolder and not isfolder("Sakura") then makefolder("Sakura") end;
                writefile("Sakura/fflags.json", json_str);
                saved = true;
            end);
        end;
...
do
    ...
    motion_blur.Name = "Sakura_motion_blur";
    motion_blur.Size = 0;
...
do
    local forced = false;
    if not Library.__SakuraDragHooked and type(Library.MouseIsOverFrame) == "function" then
        Library.__SakuraDragHooked = true;
...
local Sakura_state = { esp_hidden = false, vm_chams_last = 0 };

trove:Add(run_service.RenderStepped:Connect(LPH_NO_VIRTUALIZE(function()
	tickval = tick();

	if not ELI.esp.enable then
		if not Sakura_state.esp_hidden then
			Sakura_state.esp_hidden = true;
			for _, player in players:GetPlayers() do
...
do
    Sakura_state.dmgCallbacks = {};
    Sakura_state.dmgQueue = {};
    Sakura_state.onDamage = function(fn) Sakura_state.dmgCallbacks[#Sakura_state.dmgCallbacks + 1] = fn end;
    Sakura_state.fireDamage = function(plr, amount, worldpos)
        local q = Sakura_state.dmgQueue;
    ...
    trove:Add(run_service.Heartbeat:Connect(LPH_NO_VIRTUALIZE(function()
        ...
        for i = 1, n do
            local d = q[i];
            for _, cb in ipairs(Sakura_state.dmgCallbacks) do
...
do
    ...
    Sakura_state.onDamage(function(plr, amount, worldpos)
...
local old_tracer = tracer_effect.Play; tracer_effect.Play = function(...)
    ...
    if ELI.silent_aim.visualize and ELI.silent_aim.enable
        ...
        local st = Sakura_state.silentTarget;
        if st and (tick() - (Sakura_state.silentTargetT or 0)) < 0.2 then
...
do
local vm_material_presets = getgenv().Sakura_materials.presets;
...
trove:Add(run_service.Heartbeat:Connect(LPH_NO_VIRTUALIZE(function()
    ...
    local now = tick();
    if now - Sakura_state.vm_chams_last < 0.1 then return end;
    Sakura_state.vm_chams_last = now;
...
do
    function skybox_changer:init()
        ...
        self.list = {
            ...
            ["Sakura Sky"] = { SkyboxBk = "rbxassetid://1898724755", SkyboxDn = "rbxassetid://1898727189", SkyboxFt = "rbxassetid://1898722814", SkyboxLf = "rbxassetid://1898729298", SkyboxRt = "rbxassetid://1898741025", SkyboxUp = "rbxassetid://1898736761" },
        };
...
local replicate_from_server = local_fighter.ReplicateFromServer; local_fighter.ReplicateFromServer = function(controller, _type, ...)
    if _type == 'DamageNumberEffect' then
        ...
        if Sakura_state.fireDamage and ELI.damage_numbers.enable then
            local hp = players:GetPlayerFromCharacter(hit_character);
            local ok, pos = pcall(function()
                return hit_root.Position + Vector3.new(0, is_headshot and 2.6 or 2.1, 0);
            end);
            Sakura_state.fireDamage(hp, hit_damage, ok and pos or nil);
        end;
    ...
    if ELI.silent_aim.enable and debug.info(3, "n") == "StartShooting" then
        if math.random(1, 100) <= ELI.silent_aim.hit_chance then
            ...
            if closest and closest.Character then
                ...
                if target_pos then
                    Sakura_state.silentTarget = target_pos;
                    Sakura_state.silentTargetT = tick();
                    local args = {...};
                    args[3] = target_pos;
...
do
    ...
    local function apply(kind) pcall(function() getgenv().Sakura_apply_opt(kind) end) end;
    ...
    ff_box:AddLabel('2 it applies live if your executor supports it and always saves them to the Sakura fflags file', true);
    ...
    do
        ...
        local base = 'open its settings > fastflags editor, paste or import the Sakura fflags file then save';
        ...
        local guide;
        if name == '' then
            guide = 'could not auto detect your executor, open its fastflags or config editor, import the Sakura fflags file then fully rejoin roblox';
        else
    ...
    local function do_inject(json)
        local injector = getgenv().Sakura_inject_fflags;
        ...
        elseif res.saved then
            Library:Notify('fflags saved to Sakura/fflags.json - import in your executor & rejoin', 7);
        else
    ...
    ff_box:AddButton('save preset to file', function()
        ...
        pcall(function()
            if makefolder and isfolder and not isfolder('Sakura') then makefolder('Sakura') end;
            writefile('Sakura/fflags.json', getgenv().Sakura_fflag_preset or '{}');
        end);
        Library:Notify('saved preset to Sakura/fflags.json', 5);
    end);
end;
...
local visuals_tab = Tabs.visualstab; do
    ...
    local viewmodel_thing = visuals_tab:AddLeftTabbox(); do
        local viewmodel_tab = viewmodel_thing:AddTab('viewmodel'); do
            ...
            local vm_material_names = getgenv().Sakura_materials.names;
...
local world_tab = Tabs.worldtab; do
    ...
    shader_bloom.Name = 'Sakura_bloom';
    ...
    shader_sunrays.Name = 'Sakura_sunrays';
...
getgenv().Sakura_cosmetic_state = getgenv().Sakura_cosmetic_state or {
    gui = nil,
    frame = nil,
    inner = nil,
    panel = nil,
...
v555:AddToggle("AnySkinMode", {
    Text = "any skin",
    Default = getgenv().Sakura_cosmetic_state.anyModel or false,
    Callback = function(val)
        getgenv().Sakura_cosmetic_state.anyModel = val and true or false;
        local st = getgenv().Sakura_cosmetic_state;
...
local function openCosmeticChanger(showUi)
    ...
    local SKINS_FOLDER = "Sakura_skins";
    ...
    local uiState = getgenv().Sakura_cosmetic_state;
    ...
    gui.Name = "Sakura_cosmetic_changer";
    ...
    getgenv().Sakura_apply_cosmetic_theme = applyCosmeticUiTheme;
end;
...
;(function()
    local cfg = ELI;
    local material_presets = getgenv().Sakura_materials.presets;
    local material_preset_names = getgenv().Sakura_materials.names;
    ...
    ch_text_gui.Name = "Sakura_crosshair_text";
...
do
    ...
    local function StyleNotification()
        ...
        for _, Outer in eli_ipairs(Area:GetChildren()) do
            if Outer:IsA("GuiObject") and not Outer:GetAttribute("SakuraStyled") then
                Outer:SetAttribute("SakuraStyled", true);
    ...
    if not Library.__SakuraNotifyHooked then
        Library.__SakuraNotifyHooked = true;
        Library.__SakuraOldNotify = Library.Notify;
        ...
        function Library:Notify(...)
            ...
            ApplyNotifySettings();
            local Data = Library.__SakuraOldNotify(self, ...);
...
ThemeManager:SetFolder('Sakura');
SaveManager:SetFolder('Sakura/rivals');
SaveManager:BuildConfigSection(Tabs['UI Settings'])

;(function()
    local lua_tab = Tabs.lua;
    local base_folder = SaveManager.Folder or 'Sakura/rivals';
...
;(function()
    ...
    pcall(function()
        ...
        wmbox:AddToggle('WatermarkFps', {
            ...
            Callback = function(v)
                getgenv().Sakura_wm = getgenv().Sakura_wm or {};
                getgenv().Sakura_wm.fps = v;
            end;
        });
    ...
    pcall(function()
        ...
        getgenv().Sakura_toggle_gui = toggleGui;
        local btn = Library:Create('TextButton', {
            Name = 'SakuraMobileToggle';
    ...
    pcall(function()
        ...
        if not holder or holder:FindFirstChild('SakuraMainResizer') then return end;
        local uis = game:GetService('UserInputService');
        local handle = Instance.new('Frame');
        handle.Name = 'SakuraMainResizer';
